...
public class CameraControl : MonoBehaviour
{
    ...
    [SerializeField] Grid _gridInOut;
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.S) && !Input.GetKey(KeyCode.LeftControl) && !Input.GetKey(KeyCode.RightCommand))
        {
            transform.position -= Vector3.up * Time.deltaTime * 2 * _multiplicateurCameraSpeed;
        }
        ...
        if (!Input.GetKey(KeyCode.LeftControl))
        {
            ...
            if (scroll < 0 && (_grid.transform.localScale.x - (1 * Time.deltaTime * 12 * _multiplicateurCameraSpeed)) > 0.15) // Molette vers le bas
            {
                ...
                _gridInOut.transform.localScale -= Vector3.one * Time.deltaTime * 12 * _multiplicateurCameraSpeed;
            }
